...
## ★Codexへのキックオフ定型文（人間がコピペ）
> このリポジトリ（hippo-rocket）の AGENTS.md と HANDOFF.md を読んで現在地を把握し、HANDOFF/開発計画の「次の一手」から続きを実装して。設計を変えるなら該当docとHANDOFFも更新。コミットはするがpushはしない。敵/機体/ステージを触ったら `node tools/validate.cjs` で検証。終わる前にHANDOFFを最新化して。
